...
export default async function ApprovalsPage() {
  ...
  const btn = (bg: string, color: string): CSSProperties => ({
    ...
    color,
    background: bg,
    border: bg === 'transparent' ? `1px solid ${brand.line}` : 'none',
    borderRadius: '2px',
    cursor: 'pointer',
  });

  return (
    <AdminShell active="approvals" isAdmin={profile?.role === 'admin'}>
      ...
      >
        <div style={{ padding: '.8rem', borderBottom: `1px solid ${brand.line}`, fontWeight: 700 }}>
          Pending requests
        </div>
        {pending.length === 0 ? (
                ...
                <tr key={u.id}>
                  ...
                  <td style={{ ...cell, textAlign: 'right' }}>
                    <form action={setUserStatus} style={{ display: 'inline-flex', gap: '.4rem' }}>
                      ...
                      <button style={btn('transparent', '#8a2020')} name="status" value="rejected">
                        Reject
                      </button>
                    </form>
                  </td>
          ...
          <tbody>
            {others.map((u) => (
              <tr key={u.id}>
                <td style={cell}>{u.full_name || '—'}</td>
                <td style={cell}>{u.email}</td>
                <td style={{ ...cell, textTransform: 'capitalize' }}>{u.role}</td>
                <td style={cell}>
                  <span
                    style={{ color: u.status === 'approved' ? brand.ok : '#8a2020', fontWeight: 600 }}
                  >
                    {u.status}
                  </span>
                </td>
