...
if(t==='pricing')return '<div class="pv-sec"><div class="shead center">'+eyebrow(p.eyebrow||'Options',id+'.eyebrow',edit)+'<h2'+ce(id+'.title',edit)+'>'+accentText(p.title)+'</h2></div><div style="display:grid;grid-template-columns:repeat('+p.plans.length+',1fr);gap:14px">'+p.plans.map((pl,i)=>'<div class="pv-card'+(pl.hl?' pv-plan-hl':'')+'"><div style="font-family:var(--mono);font-size:.64rem;text-transform:uppercase;letter-spacing:.06em;color:var(--amber-2)"'+ce(id+'.plans.'+i+'.name',edit)+'>'+esc(pl.name)+'</div><div style="font-family:var(--display);font-weight:900;font-size:1.9rem;margin:6px 0"'+ce(id+'.plans.'+i+'.price',edit)+'>'+esc(pl.price)+'</div><div style="color:var(--muted);font-size:.8rem;margin-bottom:12px"'+ce(id+'.plans.'+i+'.per',edit)+'>'+esc(pl.per)+'</div>'+pl.feats.map((f,fi)=>'<div style="display:flex;gap:8px;padding:5px 0;font-size:.87rem;border-top:1px solid var(--line)"><span style="color:var(--ok)">✓</span><span'+ce(id+'.plans.'+i+'.feats.'+fi,edit)+'>'+esc(f)+'</span></div>').join('')+'<div style="margin-top:14px">'+btn(pl.cta,pl.hl?'solar':'dark ghost',false,null,id+'.plans.'+i+'.cta',edit,pl.ctaHref||'#quote')+'</div></div>').join('')+'</div></div>';
 if(t==='steps'){var _sn=p.items.length;var _sfill='';if(_sn>4&&_sn%4!==0){_sfill='<div class="pv-pstep-fill'+(_sn%2===0?' is-teven':'')+'" style="grid-column:span '+((4-_sn%4)%4)+'" aria-hidden="true"><span class="pv-pstep-fill-box"></span></div>';}return '<div class="pv-sec"><div class="shead">'+eyebrow(p.eyebrow||'How it works',id+'.eyebrow',edit)+'<h2'+ce(id+'.title',edit)+'>'+accentText(p.title)+'</h2></div><div class="pv-steps" style="grid-template-columns:repeat('+Math.min(p.items.length,4)+',1fr)">'+p.items.map((s,i)=>'<div class="pv-pstep"><div class="n">'+String(i+1).padStart(2,'0')+'</div><h4'+ce(id+'.items.'+i+'.title',edit)+'>'+accentText(s.title)+'</h4><p'+ce(id+'.items.'+i+'.text',edit)+'>'+esc(s.text).replace(/\n/g,'<br>')+'</p></div>').join('')+_sfill+'</div></div>';}
...
function openAiBuilder(mode){window._aiState={loading:false,error:'',blocks:null,prompt:'',target:(mode==='new'?'new':'append')};var box=document.getElementById('modalbox');if(box)box.className='modalbox';renderAiBuilder();var m=document.getElementById('modal');if(m)m.classList.add('show');var inp=document.getElementById('ai-prompt');if(inp)inp.focus();}
...
function aiBlockPreview(b,i){return '<div style="border:1px solid var(--line);border-radius:5px;overflow:hidden;background:var(--paper)"><div style="font-family:var(--mono);font-size:.55rem;letter-spacing:.08em;text-transform:uppercase;color:var(--muted);padding:6px 9px;border-bottom:1px solid var(--line);background:var(--paper-2)">'+esc(blockDisplayName(b.t))+'</div><div style="height:150px;overflow:hidden;pointer-events:none"><div style="width:1160px;transform:scale(.2);transform-origin:top left">'+renderBlock({t:b.t,p:b.p,id:'aiprev'+i},false)+'</div></div></div>';}
function renderAiBuilder(){var box=document.getElementById('modalbox');if(!box)return;var st=window._aiState||{};var tgt=st.target||'append';var h='<button class="close" onclick="closeModal()">×</button>';h+='<h2>AI page builder</h2>';h+='<p>Describe the page you want — a case study, a service page, an FAQ. The assistant researches the top-ranking pages for your topic, then writes a full, SEO &amp; GEO-optimised draft (with meta title/description) you can edit and drop in.</p>';h+='<div class="fld"><label for="ai-prompt">What should this page cover?</label><textarea id="ai-prompt" rows="4" placeholder="e.g. A case study about a 55 kWp warehouse rooftop install in Newport that cut the client&#39;s grid use by 42%. Cover the challenge, what we installed, the results, and a call to action.">'+esc(st.prompt||'')+'</textarea></div>';h+='<div class="fld"><label>Where should it go?</label><label class="chk"><input type="radio" name="ai-target" value="append"'+(tgt!=='new'?' checked':'')+' onchange="aiTargetChange()"> Add to the current page</label><label class="chk"><input type="radio" name="ai-target" value="new"'+(tgt==='new'?' checked':'')+' onchange="aiTargetChange()"> Create a new page</label></div>';h+='<div class="fld" id="ai-new-name-wrap" style="display:'+(tgt==='new'?'':'none')+'"><label for="ai-new-name">New page title</label><input id="ai-new-name" type="text" placeholder="e.g. Warehouse Case Study — Newport"></div>';if(st.error)h+='<div class="modal-err" role="alert">'+esc(st.error)+'</div>';if(st.loading)h+='<div class="hint">Reading the top-ranking pages and writing a full SEO/GEO-optimised page — this can take up to ~2 minutes. Keep this window open.</div>';if(st.debug)h+='<div class="hint" style="font-size:.72rem;opacity:.85">Model: '+esc(st.debug.model)+' · '+st.debug.rawCount+' sections generated, '+st.debug.keptCount+' kept after validation.'+((st.debug.keptCount<st.debug.rawCount&&st.debug.firstKeys&&st.debug.firstKeys.length)?' · block keys: '+esc(st.debug.firstKeys.join(', ')):'')+'</div>';if(st.blocks&&st.blocks.length){h+='<div class="hint">Proposed sections ('+st.blocks.length+') — preview below, then insert.</div>';h+='<div style="max-height:260px;overflow:auto;display:flex;flex-direction:column;gap:10px;margin:8px 0">'+st.blocks.map(aiBlockPreview).join('')+'</div>';}h+='<div class="modal-actions">';h+='<button type="button" class="tbtn modal-btn-ghost" onclick="closeModal()">Cancel</button>';if(st.blocks&&st.blocks.length){h+='<button type="button" class="tbtn tbtn-quiet" onclick="aiGenerate()"'+(st.loading?' disabled':'')+'>'+(st.loading?'Researching &amp; writing…':'Regenerate')+'</button>';h+='<button type="button" class="tbtn solar modal-btn-primary" onclick="aiInsert()">Insert '+st.blocks.length+' section'+(st.blocks.length>1?'s':'')+'</button>';}else{h+='<button type="button" class="tbtn solar modal-btn-primary" onclick="aiGenerate()"'+(st.loading?' disabled':'')+'>'+(st.loading?'Researching &amp; writing…':'Generate page')+'</button>';}h+='</div>';box.innerHTML=h;}
async function aiGenerate(){var st=window._aiState||(window._aiState={});var inp=document.getElementById('ai-prompt');var prompt=(inp&&inp.value||'').trim();if(prompt.length<8){st.error='Describe the page in a sentence or two first.';renderAiBuilder();return;}var sel=document.querySelector('input[name="ai-target"]:checked');st.target=sel?sel.value:'append';st.prompt=prompt;st.loading=true;st.error='';st.blocks=null;renderAiBuilder();try{var r=await fetch('/api/cms/ai-page',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({prompt:prompt,context:aiPageContext()})});var d=null;try{d=await r.json();}catch(e){}st.loading=false;st.debug=(d&&d.debug)||null;if(!r.ok){st.error=(d&&d.error)||('Generation failed ('+r.status+')');renderAiBuilder();return;}st.blocks=(d&&Array.isArray(d.blocks))?d.blocks:[];st.seo=(d&&d.seo)||null;if(!st.blocks.length)st.error='The AI did not return any usable sections. Try rephrasing your description.';renderAiBuilder();}catch(e){st.loading=false;st.error='Network error — please try again.';renderAiBuilder();}}
function aiInsert(){var st=window._aiState;if(!st||!st.blocks||!st.blocks.length)return;var sel=document.querySelector('input[name="ai-target"]:checked');var target=sel?sel.value:'append';var blocks=st.blocks.map(function(b){return {id:uid(),t:b.t,p:JSON.parse(JSON.stringify(b.p))};});var aiseo=st.seo||{};if(target==='new'){var nameInp=document.getElementById('ai-new-name');var typed=(nameInp&&nameInp.value.trim())||'';var name=uniquePageTitle(typed||aiseo.title||st.prompt.slice(0,50)||'AI page');var slug=uniquePageSlug(titleToSlug(aiseo.slug||name));STATE.pages.push({id:uid(),_draft:1,name:name,slug:slug,type:'page',seo:{slug:slug,title:(aiseo.title||name),desc:(aiseo.description||'')},blocks:blocks.concat([makeBlock('footer')])});STATE.current=STATE.pages.length-1;}else{if(!page()){closeModal();return;}var pg=page();pg.blocks=pg.blocks.concat(blocks);pg.seo=pg.seo||{};if(aiseo.title&&!pg.seo.title)pg.seo.title=aiseo.title;if(aiseo.description&&!pg.seo.desc)pg.seo.desc=aiseo.description;}SEL=blocks.length?blocks[blocks.length-1].id:null;closeModal();MODE='edit';setMode();renderAll();tab('build');save();syncCmsUrl(false);}
/* ============ MENU TAB ============ */
...
function sectionPrev(t,elm){popAt(elm,'Preview · '+BLOCKNAMES[t],renderBlock({t,p:SAMPLES[t]||{},id:'prev'}));}
/* ===== case studies ===== */
...
function dashNewPage(){var name=uniquePageTitle('Untitled page');var slug=uniquePageSlug(titleToSlug(name));STATE.pages.push({id:uid(),_draft:1,name:name,slug:slug,type:'page',seo:{slug:slug},blocks:[{id:uid(),t:'hero',p:{eyebrow:'New page',headline:'Your headline here',sub:'Start building — add sections from the left.',dark:true,ctaLabel:'Get a quote',ctaPulse:false,cta2:''}},makeBlock('footer')]});editPage(STATE.pages.length-1);save();}
function openNewPageChooser(){var box=document.getElementById('modalbox');var modal=document.getElementById('modal');if(!box||!modal)return;box.className='modalbox';box.innerHTML='<button class="close" onclick="closeModal()">×</button>'+'<h2>Add a new page</h2>'+'<p>Start from a blank page, or let the AI research your topic and draft the whole page.</p>'+'<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:16px">'+'<button type="button" class="dact dact-icon" onclick="closeModal();dashNewPage()"><span class="dact-ic">'+icon('home',22)+'</span><b>Blank page</b><span>An empty page with a hero to build on.</span></button>'+'<button type="button" class="dact dact-icon" onclick="closeModal();openAiBuilder(\'new\')"><span class="dact-ic">'+icon('solar',22)+'</span><b>AI page builder</b><span>Describe it — the AI researches and writes a full draft.</span></button>'+'</div>';modal.classList.add('show');}
...
function renderMega(){var el=document.getElementById('megaedit');if(!el)return;var site=STATE&&STATE.site;var M=(site&&site.menu)||[];
 ...
 if(mega){
   ...
   var featured='<div class="mm-featured'+(feat.bg==='light'&&!_featSrc?' light':'')+'"'+fbg+'>'+(_featSrc?'':'<svg class="mm-spark" viewBox="0 0 24 24" fill="#F8BC1E" width="32" height="32">'+[0,90,180,270].map(function(a){return '<g transform="rotate('+a+' 12 12)">'+RAY+'</g>'}).join('')+'</svg>')+'<div class="mm-ft-t">'+esc(feat.title)+'</div><p>'+esc(feat.text).replace(/\n/g,'<br>')+'</p><span class="mm-ft-cta">'+esc(feat.cta)+' →</span></div>';
...
w.openNewPageChooser = openNewPageChooser;
