...
function renderBlock(b,edit){
 ...
 if(b.t==='cta')return '<div class="pv-cta"><div class="z"><h2'+ce(id+'.headline',edit)+'>'+esc(p.headline)+'</h2><p'+ce(id+'.sub',edit)+'>'+esc(p.sub)+'</p><div class="pv-btnrow" style="justify-content:center">'+btn(p.btn,'solar',p.pulse,null,id+'.btn',edit)+'</div></div></div>';
...
if(b.t==='media'){const im=p.img?imgTag(p.img,'width:100%;border-radius:3px;display:block',edit,id+'.img'):'<div style="aspect-ratio:4/3;background:linear-gradient(135deg,#26324c,#171d2b);border-radius:3px;display:grid;place-items:center;color:var(--muted-d);font-family:var(--mono);font-size:.7rem">IMAGE</div>';
   const tx='<div>'+eyebrow(p.eyebrow,id+'.eyebrow',edit)+'<h2 style="font-size:clamp(1.5rem,2.6vw,2.1rem);font-weight:800;margin-top:10px"'+ce(id+'.title',edit)+'>'+esc(p.title)+'</h2><p style="color:var(--muted);margin-top:12px;line-height:1.6"'+ce(id+'.text',edit)+'>'+esc(p.text)+'</p><div class="pv-btnrow">'+btn(p.cta,'solar',false,null,id+'.cta',edit)+'</div></div>';
   return '<div class="pv-sec" style="display:grid;grid-template-columns:1fr 1fr;gap:34px;align-items:center">'+(p.side==='left'?im+tx:tx+im)+'</div>';}
...
if(b.t==='pricing')return '<div class="pv-sec"><div class="shead center">'+eyebrow(p.eyebrow||'Options',id+'.eyebrow',edit)+'<h2'+ce(id+'.title',edit)+'>'+esc(p.title)+'</h2></div><div style="display:grid;grid-template-columns:repeat('+p.plans.length+',1fr);gap:14px">'+p.plans.map((pl,i)=>'<div class="pv-card'+(pl.hl?' pv-plan-hl':'')+'"><div style="font-family:var(--mono);font-size:.64rem;text-transform:uppercase;letter-spacing:.06em;color:var(--amber-2)"'+ce(id+'.plans.'+i+'.name',edit)+'>'+esc(pl.name)+'</div><div style="font-family:var(--display);font-weight:900;font-size:1.9rem;margin:6px 0"'+ce(id+'.plans.'+i+'.price',edit)+'>'+esc(pl.price)+'</div><div style="color:var(--muted);font-size:.8rem;margin-bottom:12px"'+ce(id+'.plans.'+i+'.per',edit)+'>'+esc(pl.per)+'</div>'+pl.feats.map((f,fi)=>'<div style="display:flex;gap:8px;padding:5px 0;font-size:.87rem;border-top:1px solid var(--line)"><span style="color:var(--ok)">✓</span><span'+ce(id+'.plans.'+i+'.feats.'+fi,edit)+'>'+esc(f)+'</span></div>').join('')+'<div style="margin-top:14px">'+btn(pl.cta,pl.hl?'solar':'dark ghost',false,null,id+'.plans.'+i+'.cta',edit)+'</div></div>').join('')+'</div></div>';
 ...
 if(b.t==='rich')return '<div class="pv-rich">'+(p.html||'<p>Rich text…</p>')+'</div>';
 return '';
}
...
function epSet(el){const ep=el.getAttribute('data-ep');if(!ep)return;const parts=ep.split('.');const bl=findBlock(parts[0]);if(!bl)return;let o=bl.p;for(let i=1;i<parts.length-1;i++){o=o[parts[i]];if(o==null)return;}o[parts[parts.length-1]]=el.textContent;}
function epInput(el){epSet(el);debSave();}
function epBlur(el){epSet(el);renderBuild();save();}
function epKey(e){if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();e.target.blur();}}
...
function inspector(bl){const p=bl.p,id=bl.id;let h='';
 if(bl.t==='hero'){h+=txt('Eyebrow',p.eyebrow,id+'.eyebrow')+area('Headline',p.headline,id+'.headline')+area('Subheadline',p.sub,id+'.sub')+txt('Primary button',p.ctaLabel,id+'.ctaLabel')+chk('Pulse animation on button',p.ctaPulse,id+'.ctaPulse')+txt('Secondary button',p.cta2,id+'.cta2')+chk('Dark hero',p.dark,id+'.dark');}
 ...
 else if(bl.t==='cta'){h+=area('Headline',p.headline,id+'.headline')+area('Subtext',p.sub,id+'.sub')+txt('Button',p.btn,id+'.btn')+chk('Pulse animation on button',p.pulse,id+'.pulse');}
...
function selectBlock(id){SEL=id;tplHide();renderPreview();renderBuild();}
function blockDefs(){const defs={
  hero:{eyebrow:'Eyebrow',headline:'Your headline here',sub:'Supporting sentence.',dark:true,ctaLabel:'Get a quote',ctaPulse:false,cta2:''},
  ...
  cta:{headline:'Ready to start?',sub:'Book a free survey today.',btn:'Get my free quote',pulse:true},
  faq:{items:[{q:'A question?',a:'An answer.'}]},
  rich:{html:'<p>Write anything here…</p>'},
  media:{img:'',side:'right',eyebrow:'Why choose us',title:'A section title',text:'Describe the benefit here — pair it with a real photo of your work.',cta:'Learn more'},
